    ...
    for (i = 0; i < select.length; i++) {
      if (select.options[i].selected) {
        ...
        if (json.locations.length == 2) {
          ...
          // $rootScope.$emit( 'map.dropOriginMarker', [geo.olat, geo.olon],
          // mode);
          Locations.push({
            lat : geo.dlat,
            lon : geo.dlon
          })
          // $rootScope.$emit( 'map.dropDestMarker', [geo.dlat, geo.dlon],
          // mode);
          // locations++;
          var waypoints = [];
          ...
          via_array.forEach(function(via_array, i) {
            ...
            // $rootScope.$emit( 'map.dropViaMarker', [via_array.vlat,
            // via_array.vlon], mode)
          });
          waypoints.push(L.latLng(geo.dlat, geo.dlon));
          // locations++;
        ...
        if (typeof json.locations[0].date_time != "undefined")
          options.date_time = json.locations[0].date_time;
        ...
        var rr = L.Routing.control({
          ...
          router : L.Routing.valhalla(envToken, json.costing, options),
