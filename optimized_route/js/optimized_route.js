let app = angular.module('optimized_route', [])
var hash_params = L.Hash.parseHash(location.hash)

var serviceUrl
var envServer = 'production';
let envToken = accessToken.prod
...
function handleChange (evt) {
  ...
  for (let i = 0; i < sel.options.length; i++) {
    let results = sel.options[i].text + '  ' + sel.options[i].value
...
app.controller('OptimizedRouteController', function ($scope, $rootScope, $sce, $http) {
  ...
  let getDestinationIcon = function () {
    return new L.Icon({
  ...
  let parseParams = function (pieces) {
    let parameters = {}
  ...
  let updateHashCosting = function (costing, costingOptions, directionsOptions, dateTime) {
    // update the permalink hash
    let pieces = parseHash()
  ...
  let hashRoute = function () {
    // something has to have changed for us to request again
    let parameters = parseParams(parseHash())
...
: {
  ...
  optimizeBtn.addEventListener('click', function (e) {
    ...
    selectEnv()
    rr = createRouting({ waypoints, costing: $scope.mode })
...
//locate edge snap markers
  var locateEdgeMarkers = function (locate_result) {
    ...
    if (locate_result.node != null) {
      ...
      locate_result.edges.forEach(function (element, index, array) {
        let marker = L.circle([element.correlated_lat, element.correlated_lon], 2, { color: '#444', opacity: 1, fill: true, fillColor: '#eee', fillOpacity: 1 })
        map.addLayer(marker)
        var popup = L.popup({ maxHeight: 200 })
  ...
  function getNextTuesday () {
    let today = new Date(); var day; var tuesday
    day = today.getDay()
  ...
  $(document).on('route:time_distance', function (e, td) {
    let instructions = $('.leaflet-routing-container.leaflet-control').html()
