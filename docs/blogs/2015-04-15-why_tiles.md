...
Ok, so the above is slightly embellished (everyone knows Dave doesn't use the phone since that one weekend call about traffic on the NJ Turnpike), but let's just say Kevin's proposal was met with some degree of skepticism. But after reasoned consideration the team decided that tiled, hierarchical routing data had merits and was worth pursuing.
